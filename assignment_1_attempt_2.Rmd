...
##Create a column that includes just month and day in data.filtered (data.filtered2) and blpw.band (blpw.band2)

```{r}
data.filtered$monthday <- paste(month.abb[data.filtered$month],data.filtered$day,sep = "-")

data.filtered2 <- data.filtered %>%
group_by(band) %>%
arrange(band,monthday) %>%
data.frame()


blpw.band$monthday <- paste(month.abb[blpw.band$month],blpw.band$day,sep="-")
...
```{r}
ggplot(data=data.filtered2,mapping=aes(x=monthday,y=diff_mass,colour=band))+
+ geom_point()+
+ geom_line()+
